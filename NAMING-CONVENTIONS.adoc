...
*Use hyphens:* `+tree-navigator+` - URLs:
`+gitlab.com/group/tree-navigator+` - Clone:
`+git clone git@gitlab.com:group/tree-navigator.git+` - Directory:
`+cd tree-navigator+`
...
*Use hyphens:* - DEB: `+tree-navigator_2.0.0_amd64.deb+` - RPM:
`+tree-navigator-2.0.0-1.x86_64.rpm+` - Tarball:
`+tree-navigator-2.0.0-linux-x86_64.tar.gz+`
...
[arabic]
. *Hyphens for user-facing names*: Easier to type, URL-friendly, modern
convention
. *Underscores for Ada*: Required by Ada language specification for file
names
. *Underscores for environment variables*: POSIX shell convention
(hyphens not allowed)
. *Consistency within each domain*: URLs all use hyphens, Ada all uses
underscores
